...
<body onload="set_theme()">
        ...
        <nav class="menu" role="navigation">
            <div class="inner">
                <div class="m-left" style="color:white;">
                        ...
                        <ul>
                            <li class="menu-deroulant">
                                ...
                                <ul class="sous-menu">
                                    ...
                                    <li><a href="{{ route('service', 'Relations Entreprise') }}">Relations
                                            Entreprise</a></li>
                    ...
                    @super
                    <a onclick="admin(`{{ $service }}`)"><i class="fas fa-plus fa-2x" style="color:white; margin-top:5%;"></i></a>
                    @endsuper
                    ...
                    <a onclick="changeColor();" class="image_button">
                        <img src="../picture/moon.png" style="width:50px;">
                    </a>
                </div>
    ...
    <div class="ecran">
        <div class="left">
            <div class="recherche">
                <input type="text" name="search" id="barre" onkeyup="search()" placeholder="Faire une recherche"> <i class="fas fa-search" style="color:white;"></i>
            </div>
            <div class="photo">
                <div class="chef" style="flex-direction: row; display:flex;">
                    @foreach ($users as $user)
                    @if (str_contains($user->service, $service) && str_contains($user->rights, 'super adm'))
                    <a onclick="profiler(`{{ $user->login }}`)">
                        <div class="student">
                            ...
                            <img src="https://auth.etna-alternance.net/api/users/{{ $user->login }}/photo" style="width:100px;">
                            <h4>{{ $user->login }}</h4>
                        </div>
                    </a>
                    @endif
                ...
                <div class="employe" style="flex-direction:row; display:flex; flex-wrap: wrap;">
                    @foreach ($users as $user)
                    @if (str_contains($user->service, $service) && str_contains($user->rights, 'super adm') == false)
                    <a onclick="profiler(`{{ $user->login }}`)">
                        <div class="student">
                            <div hidden>
                                {{ strtoupper($user->nom) }}{{ strtolower($user->nom) }}{{ $user->nom }}
        ...
        <div class="right">
            <iframe sandbox="allow-scripts allow-same-origin allow-forms allow-top-navigation" id="profilediv" src="" width="100%" height="100%" frameBorder="0" sandbox>
                <p></p>
            </iframe>
        </div>
    </div>
